...
def task(i,testdatas,  users, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,shareitems, itemreceive, responseitems):
    try:
        submitresAll = []
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    results = Parallel(n_jobs=threadingNum,verbose=50,backend="loky",prefer="threads")(delayed(task)(i,testjsonDataSplits[i],  usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,
          shareitems, itemreceive, responseitems) for i in range(len(testjsonDataSplits)))
    start3 = time.time()
