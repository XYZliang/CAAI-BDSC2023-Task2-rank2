...
def task(i,testdatas,  users, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,
    ...
    try:
        for testdata in tqdm(testdatas):
            print("我是进程"+i+",正在处理的用户是" + testdata['inviter_id'])
...
# 主函数
def main(verification=False, percentage=0.1, savaPath="./output/result/", Hyperthreading=True, MaxThreading=4):
    # 文件路径
    file_paths = {
    ...
    print(str(cpu_count) + "核心" + str(cpu_count_logical) + "线程" + str(free_memory) + "G内存" + "，开启任务进程数" + str(
        threadingNum))
    if free_memory < 12:
    ...
    pool = pathos.pp.ParallelPool(ncpus=threadingNum)
    # 初始化进程所需要的共享数据
    usersMini = {}
    ...
    for i in range(len(testjsonDataSplits)):
        pool.apipe(task,i, testjsonDataSplits[i],usersMini, dataLF, dataItemLF, itemsinfo, netrelation,
                    sharerank,userSimilarityMatrix, shareitems, itemreceive, responseitems, submitres, submitresAll)
