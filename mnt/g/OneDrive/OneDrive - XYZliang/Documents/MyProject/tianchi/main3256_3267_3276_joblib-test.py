...
def task(i,testdatas,  users, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,shareitems, itemreceive, responseitems, num):
    def saveDcore(id1, fid, r):
        # 将评分（r）添加到 SortedDict scor 中
        nonlocal scor, userSimilarityMatrix
        if -r not in scor:
            scor[-r] = []
        scor[-r].append(fid)
    ...
    try:
        ...
        # for testdata in testdatas:
            ...
            for fid in users[id1].SimLusers.keys():
                r = getScore(id1, fid)
                saveDcore(id1, fid, r)
                for ffid in users[fid].SimLusers.keys():
                    if ffid == id1:
                        continue
                    rr = getScore(fid, ffid)
                    saveDcore(id1, ffid, r*rr)
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    verificationData = copy.deepcopy(testjsonData)
    if verification:
        # add triple_id
    ...
    #####################陈老师的原始方案 0.3256####################
    results = Parallel(n_jobs=threadingNum,verbose=50,backend="loky",prefer="threads")(delayed(task)(i,testjsonDataSplits[i],  usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,
          shareitems, itemreceive, responseitems, Num=15) for i in range(len(testjsonDataSplits)))
    start3 = time.time()
