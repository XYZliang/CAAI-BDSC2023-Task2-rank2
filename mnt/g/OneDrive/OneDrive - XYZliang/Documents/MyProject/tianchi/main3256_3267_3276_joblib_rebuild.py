        ...
        for sid in userSimilarityMatrix[id1]:
            ...
            if sid in userSimilarityMatrix:
                for ssid in userSimilarityMatrix[sid]:
                    rr = userSimilarityMatrix[sid][ssid] * r
                    if -rr not in scor:
                        scor[-rr] = []
                    scor[-rr].append(ssid)
        scor = dict(sorted(scor.items()))
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank) for i in range(len(testjsonDataSplits)))
    end2 = time.time()
    ...
    results2 = Parallel(n_jobs=threadingNum, verbose=50, backend="loky", prefer="threads")(
        delayed(task2)(i,testjsonDataSplits[i],  usersMini,shareitems,itemreceive,responseitems,userSimilarityMatrix) for i in range(len(testjsonDataSplits)))
    end3 = time.time()
