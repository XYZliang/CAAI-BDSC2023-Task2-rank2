...
def task(i, testdatas, users, dataLF, dataItemLF, itemsinfo, netrelation, sharerank, shareitems, itemreceive,
    ...
    for testdata in pbar:
        ...
        for fid in users[id1].SimLusers.keys():
            ...
            def calculate_similarity(fid, itemid, attribute, weight):
                if itemid in itemArr2User[attribute].keys():
                    if "age" in itemArr2User[attribute][itemid].keys():
                        aage = itemArr2User[attribute][itemid]["age"]
                        agen = itemArr2User[attribute][itemid]["gen"]
                        uage = users[fid].Age
                        uagen = users[fid].Gender
                        dist = 2 ** 0.5 - math.sqrt((aage - uage) ** 2 + (agen - uagen) ** 2)
                    else:
                        dist = 1
                else:
                    dist = 1
            ...
            for attribute, weight in attributes_weights.items():
                id = itemsinfo.get(attribute)
                iusim *= calculate_similarity(fid, id, attribute, weight)

            # 计算总评分（r）
            r = iusim * sir * rsim * ritem * \
            ...
            #                 tr = getPfromPdf(userPList[fid]['pdfW'],mins)
            #                 # bouth(Alist[0],userPList[fid]['pdfW'])
            #             else:
            #                 tr = getPfromPdf(Alist[0],mins)
            #         else:
            #             tr = getPfromPdf(Alist[0],mins)
            #     else:
            #         if "pdfH" in userPList[fid].keys():
            #             if userPList[fid]['pdfH']!=None:
            #                 tr = getPfromPdf(userPList[fid]['pdfH'],mins)
            #                 # bouth(Alist[1],userPList[fid]['pdfH'])
            #             else:
            #                 tr = getPfromPdf(Alist[1],mins)
            #         else:
            #             tr = getPfromPdf(Alist[1],mins)
            #     r = (tr ** 4) * r
            # else:
        ...
        subdata = {
            ...
            "item_id": str(testdata["item_id"]),
            "candidate_voter_list": res,
        }
        submitresAll.append(subdata)
    # # 保存all
    # with open("./r"+str(i)+".json", "wb") as f:
    #     f.write(orjson.dumps(all),option=orjson.OPT_INDENT_2)
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    # 文件路径
    file_paths = {
        "分享数据": "./data/item_share_train_info_new.json",
    ...
    for id1, userInfo in tqdm(users.items(), desc="正在处理无效用户数据"):
        # 记录是否有修改
        isChange = False
        # 在这里处理 userInfo 对象
    ...
    for temp in tqdm(jsonData):
        ...
        if item_id not in itemArr2UserList['item_id']:
            itemArr2UserList['item_id'][item_id] = []
        itemArr2UserList['item_id'][item_id].append(voter_id)
    ...
    for temp in tqdm(itemjsonData):
        ...
        if itemid not in itemArr2UserList['item_id'].keys():
            itemArr2UserList['item_id'][itemid] = []

        if cate_id not in itemArr2UserList["cate_id"].keys():
            itemArr2UserList["cate_id"][cate_id] = []

        if level_id not in itemArr2UserList["cate_level1_id"].keys():
            itemArr2UserList["cate_level1_id"][level_id] = []

        if brandid not in itemArr2UserList["brand_id"].keys():
            itemArr2UserList["brand_id"][brandid] = []

        if shopid not in itemArr2UserList["shop_id"].keys():
            itemArr2UserList["shop_id"][shopid] = []

        userss = itemArr2UserList['item_id'][itemid]
        itemArr2UserList["cate_id"][cate_id].extend(userss)
        itemArr2UserList["cate_level1_id"][level_id].extend(userss)
        itemArr2UserList["brand_id"][brandid].extend(userss)
        itemArr2UserList["shop_id"][shopid].extend(userss)
    ...
    print(itemArr2UserList)

    def calc_age_gender(category):
        for id in tqdm(itemArr2UserList[category].keys(), desc="正在处理" + category + "的相关用户数据"):
            age, gender = 0, 0
            if len(itemArr2UserList[category][id]) >= 8:
                for user_id in itemArr2UserList[category][id]:
                    if age < 0:
                        print(user_id + "age错误")
                    if gender < 0:
                        print(user_id + "gender错误")
                    age += users[user_id].Age
                    gender += users[user_id].Gender
                age /= len(itemArr2UserList[category][id])
                gender /= len(itemArr2UserList[category][id])
                itemArr2User[category][id] = {
                    "age": age,
                    "gen": gender
                }
            else:
    ...
    # 使用新的函数处理"item_id"和"cate_id"
    calc_age_gender("item_id")
    calc_age_gender("cate_id")
    calc_age_gender("cate_level1_id")
    calc_age_gender("brand_id")
    calc_age_gender("shop_id")
    del itemArr2UserList
    ...
    print(str(cpu_count) + "核心" + str(cpu_count_logical) + "线程" + str(
        ...
        threadingNum))
    testjsonDataSplits = [[] for _ in range(threadingNum + 3)]
    # 尽可能的使得每个线程处理的数据量相近，同时保证 inviter_id 一样的数据划分在同一组
    for i, group in enumerate(sorted_groups):
        # 找到当前最小的数据集
        smallest_dataset_index = min(range(threadingNum + 3), key=lambda index: len(testjsonDataSplits[index]))
        # 将数据添加到最小的数据集
    ...
    for userId, user in tqdm(users.items()):
        ...
        usersMini[userId].Gender = user.Gender
        usersMini[userId].Age = user.Age
    ...
    results = Parallel(n_jobs=threadingNum, verbose=50, backend="loky", prefer="threads")(
        delayed(task)(i, testjsonDataSplits[i], usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,
                      shareitems, itemreceive, responseitems, itemArr2User) for i in range(len(testjsonDataSplits)))
    start3 = time.time()
        ...
        for key, sub_dict in d.items():
            ...
            for sub_key, value in sub_dict.items():
                if sub_key in userSimilarityMatrix[key]:
                    ...
                    userSimilarityMatrix[key][sub_key] = value
    # save
    with open(savaPath + "userSimilarityMatrix.json", "w") as f:
        orjson.dumps(userSimilarityMatrix, f)
    # 将外层词典的键排序
    ...
    for res in tqdm(submitresAll):
        ...
        if len(voters) < 5:
            ...
            for voter in voters:
                ...
                for voter2 in submitDict[voter]:
                    ...
                    if voter2 not in voters:
                        ...
                        for attribute, weight in attributes_weights.items():
                            id = itemsinfo.get(attribute)
                            iusim *= calculate_similarity(voter2, itemid, attribute, weight)
                        score = score1 * score2 * iusim
                        ...
                        if voter2 in userSimilarityMatrix.keys():
                            for voter3 in userSimilarityMatrix[voter2].keys():
                                if voter3 not in voters and voter3 != userID and voter3 not in voters:
                                    ...
                                    for attribute, weight in attributes_weights.items():
                                        id = itemsinfo.get(attribute)
                                        iusim *= calculate_similarity(voter3, itemid, attribute, weight)
                                    score = score1 * score2 * score3 * iusim
            ...
            # if len(newVoters) < 5:
            #     print(userID+"补全后依旧不足五人")
            res["candidate_voter_list"] = voters
