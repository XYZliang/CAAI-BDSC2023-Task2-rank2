...
namespace tianchi
{
    internal class Program : Form
    {
        ...
        private void NewSumbit()
        {
            ...
            var netw = new Dictionary<string, HashSet<string>>();
            ...
            var SingleShare = new Dictionary<string, int>();
            var sharereceipIndex =
            ...
            //sharetrainitem[id1][itemid].Add(d);
            foreach (var iid in sharetrainitem.Keys)
            {
                SingleShare.Add(iid, 0);
                foreach (var fid in sharetrainitem[iid].Keys) SingleShare[iid] += sharetrainitem[iid][fid].Count;
            ...
            {
                id1 = testdata[0].ToString();
                itemid = testdata[1].ToString();
                var testdate = DateTime.Parse(testdata[2].ToString());
            ...
            foreach (var ite in itemreceive.Keys)
                //itemid
            foreach (var xid in itemreceive[ite].Keys)
                //id1
                //  if (!IDTest.Contains(xid)) continue;
            foreach (var yid in itemreceive[ite][xid])
                //id2
                if (itemreceive[ite].ContainsKey(yid) && itemreceive[ite][yid].Contains(xid))
                {
                    if (!shareresptimes.ContainsKey(xid))
                        shareresptimes.Add(xid, new Dictionary<string, int>());
            ...
            //}
            var ppt = new System.IO.StreamWriter("ppt.txt");


            foreach (var id in details.Keys)
            foreach (var xid in details[id].Keys)
                if (details[id][xid].Count * 1.0 / details[id][xid].ToHashSet().Count() > 1.5 &&
                    details[id][xid].Count > 4)
                {
                    if (!netw.ContainsKey(id)) netw.Add(id, new HashSet<string>());
                    foreach (var dx in ranks[id][xid].Keys)
                    foreach (var mid in ranks[id][xid][dx])
                        netw[id].Add(mid);
                }
            ...
            foreach (var id in netw.Keys)
            {
                double xy = 0;
                foreach (var fid in netw[id])
                {
            ...
            data.Clear();
            double MRR = 0;
            var loop = new Dictionary<string, Dictionary<string, int>>();
            foreach (var xid in IDTest) loop.Add(xid, new Dictionary<string, int>());
            foreach (JsonData testdata in testjsonData)
            {
                ...
                id1 = linfo.UserID = testdata[0].ToString();
                id2 = testdata[3].ToString();
                itemid = linfo.ItemID = testdata[1].ToString();
                var testdate = DateTime.Parse(testdata[2].ToString());
                ...
                // var mn = res.Count;
                // var strres = id1 + "\t" + itemid + "\t" + id2 + "\t" + allNeigbors[id2]["L"].Count + "\t" +
                //              testdate.ToString();
                // //strres = strres + "\t[" + sharerank[id1][id2].ToString() + "]";
                // //strres = strres + ",CateLevelOneId=" + users[id2].Ratio[0][itemsinfo[itemid].CateLevelOneId].ToString() + ";";
                // //strres = strres + ",CateId=" + users[id2].Ratio[1][itemsinfo[itemid].CateId].ToString() + ";";
                // //strres = strres + ",ShopId=" + users[id2].Ratio[2][itemsinfo[itemid].ShopId].ToString() + ";";
                // //strres = strres + ",BrandId=" + users[id2].Ratio[3][itemsinfo[itemid].BrandId].ToString() + "]";
                // var miflag = false;
                // //if (!Sharecorr.ContainsKey(id1)) Sharecorr.Add(id1, new Dictionary<string, List<int>>());
                // //if (!Sharecorr[id1].ContainsKey(itemid))
                // //{
                // //    Sharecorr[id1].Add(itemid, new List<int>());
                // //    Sharecorr[id1][itemid].Add(0); Sharecorr[id1][itemid].Add(0);
                // //}
                ...
                //         strres = strres + "\t" + (1 + mi).ToString();
                //
                //         miflag = true;
                //         //Sharecorr[id1][itemid][0]++;
                //         break;
                //     }
                // //Sharecorr[id1][itemid][1]++;
                //
                // foreach (var xid in res)
                // {
                //     //strres = strres + "\t" + xid+"["+ sharerank[id1][xid].ToString()+"]";
                //     //strres = strres + ",CateLevelOneId=" +  users[xid].Ratio[0][itemsinfo[itemid].CateLevelOneId].ToString() + ";";
                //     //strres = strres + ",CateId=" + users[xid].Ratio[1][itemsinfo[itemid].CateId].ToString() + ";";
                //     //strres = strres + ",ShopId=" + users[xid].Ratio[2][itemsinfo[itemid].ShopId].ToString() + ";";
                //     //strres = strres + ",BrandId=" + users[xid].Ratio[3][itemsinfo[itemid].BrandId].ToString() + "]";
                // }
                //
                // ppr.WriteLine(strres);
            }

            // MRR = MRR / testjsonData.Count;
