...
def task(i,testdatas,  users, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,userIndices):
    try:
        userSimilarityMatrix = np.zeros((len(userIndices), len(userIndices)), dtype=np.float64)
        ...
        # for testdata in testdatas:
            ...
            for fid in users[id1].SimLusers.keys():
                ...
                if -r not in scor:
                    scor[-r] = []
                scor[-r].append(fid)
                userSimilarityMatrix[userIndices[id1]][userIndices[fid]] = r
        return {id1: {fid: r}}
    except Exception:
        print(traceback.format_exc())

def task2(i,testdatas,  users,shareitems,itemreceive,responseitems):
    submitresAll=[]
    pbar = tqdm(testdatas, position=1, desc=f"进程 {i} 正在启动。。。。")
    for testdata in pbar:
        # for testdata in testdatas:
...
def main(verification=False, percentage=0.15, savaPath="./output/result/", Hyperthreading=True, MaxThreading=10):
    ...
    threadingNum = round((0.8 * threadingNum/2 + 0.2* free_memory/1.1)+1)
    if MaxThreading != 0:
    ...
    start2 = time.time()
    # 开始多进程
    userIndices = {user: index for index, user in enumerate(users)}
    userSimilarityMatrix = np.zeros((len(users), len(users)), dtype=np.float64)
    #####################陈老师的原始方案 0.3256####################
    results = Parallel(n_jobs=threadingNum,verbose=50,backend="loky",prefer="threads")(delayed(task)(i,testjsonDataSplits[i],  usersMini, dataLF, dataItemLF, itemsinfo, netrelation, sharerank,userIndices) for i in range(len(testjsonDataSplits)))
    start3 = time.time()
    ...
    #     userSimilarityMatrix[key] = {k: v for k, v in sorted(userSimilarityMatrix[key].items(), key=lambda item: item[0])}
    for d in results:
        for key, sub_dict in d.items():
            for sub_key, value in sub_dict.items():
                userSimilarityMatrix[userIndices[key]][userIndices[sub_key]] += value
    #
    # print(len(submitres))
    # print(userSimilarityMatrix)
    results2 = Parallel(n_jobs=threadingNum, verbose=50, backend="loky", prefer="threads")(
        delayed(task2)(ii,testjsonDataSplits[i],  users,shareitems,itemreceive,responseitems) for i in range(len(testjsonDataSplits)))
    # 初始化一个名为 submitres 的 List，用于存储提交的结果
    submitresAll = np.concatenate(results)
...
if __name__ == "__main__":
    main(savaPath="./output/numpyresult/")
