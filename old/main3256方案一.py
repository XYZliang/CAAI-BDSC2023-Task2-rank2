...
def find_minimum_time_difference(thisShareTime, lastShareTimes):
    format = "%Y-%m-%d %H:%M:%S"
    datetime_this = thisShareTime

    min_difference = float('inf')  # 初始设置为无穷大

    for time_str in lastShareTimes:
        datetime_last = time_str
        if datetime_last == datetime_this:
            continue
        timedelta = datetime_this - datetime_last
        difference_in_seconds = abs(timedelta.total_seconds())
        min_difference = min(min_difference, difference_in_seconds)
...
def main():
    ...
    userGenderAverage = [val / userGenderAverageCount for val in userGenderAverageSum]
    ...
    print("用户平均性别独热编码：[{}, {}]".format(userGenderAverage[0], userGenderAverage[1]))
    # 输出无效年龄、等级、性别数量
    ...
    for temp in tqdm(jsonData):
        ...
        # timestamp存储为infos[user_id][item_id][voter_id]
        if user_id not in infos:
            infos[user_id] = {}
        if item_id not in infos[user_id]:
            infos[user_id][item_id] = {}
        if voter_id not in infos[user_id][item_id]:
            infos[user_id][item_id][voter_id] = []
        infos[user_id][item_id][voter_id].append(datetime.strptime(timestamp, "%Y-%m-%d %H:%M:%S"))
    ...
    for d in tqdm(data.keys()):
        ...
        for llk in data[d]:
            ...
            # 更新回流用户与分享用户的关系
            responseitems.setdefault(id2, {}).setdefault(id1, set()).add(itemid)
    ...
    for iid in tqdm(users.keys()):
        ...
        for ii in range(4):
            ...
            users[iid].Ratio[ii] = {xx: catenum[ii][xx] / tt for xx in catenum[ii].keys()}
        ...
        users[iid].responseTimeZone = {it: timetemp[it] / timetotal for it in timetemp.keys()}
    ...
    # 创建一个名为 itemreceived 的字典，用于存储每个物品在特定日期被接收的信息
    itemreceived = {}
    ...
    for testdata in tqdm(testjsonData):
        ...
        testdate = datetime.strptime(testdata['timestamp'], '%Y-%m-%d %H:%M:%S')
    ...
    # 初始化字典和列表
    receivedata = defaultdict(set)
    itemreceived = defaultdict(lambda: defaultdict(set))
    # 确保不是重复的分享
    lastShareItemsTimes = {}
    # 定义重复分享的时间间隔（s）
    MaxLastShareTime = 60
    ...
    ShareTimePass = []
    # 遍历测试数据集 testjsonData 中的每条数据
    for testdata in tqdm(testjsonData,desc="打分中"):
        ...
        # 提取物品 ID（itemid）
        itemid = testdata['item_id']
        # 如果lastShareItemsTimes不包含id1，则加入
        if id1 not in lastShareItemsTimes:
            lastShareItemsTimes[id1]={}
        # 如果lastShareItemsTimes[id1]不包含iteamid，则加入
        if itemid not in lastShareItemsTimes[id1]:
            lastShareItemsTimes[id1][itemid]={}
        # 将数据中的时间戳转换为 DateTime 对象（testdate）
        testdate = datetime.strptime(testdata['timestamp'], "%Y-%m-%d %H:%M:%S")
        # 如果 receivedata 字典中尚未包含物品 itemid，则添加一个新条目
        if itemid not in receivedata:
            receivedata[itemid] = set()
        # 如果 users 字典中尚未包含用户 id1，则跳过此次循环
        if id1 not in users:
            continue
        # 如果 itemreceived 字典中尚未包含物品 itemid，则添加一个新条目
        if itemid not in itemreceived:
            itemreceived[itemid] = defaultdict(set)
        # 如果 itemreceived[itemid] 字典中尚未包含 testdate.Date，则添加一个新条目
        if testdate.date() not in itemreceived[itemid]:
            itemreceived[itemid][testdate.date()] = set()
        ...
        for fid in users[id1].SimLusers.keys():
            ...
            if fid in users[id1].Neighbor:
                for it in users[id1].Neighbor[fid]:
                    tsp = (testdate - it).total_seconds() / 86400  # convert seconds to days
                    rsim += math.exp(-0.02 * tsp)
                rresponse = (testdate - max(users[id1].Neighbor[fid])).total_seconds() / 86400
        ...
        wrt = f"{id1}\t{itemid}\t{len(shareitems[id1][itemid])}\t{len(users[id1].Neighbor.keys())}\t{testdate}\t{len(users[id1].SimLusers)}"
        # if len(scor) < 5:
        #     print(f"用户{id1}的相似用户不足5个")
        for xx in scor.keys():
            for yid in scor[xx]:
                # infos[id1][itemid][yid]
                if id1 not in infos:
                    infos[id1] = {}
                if itemid not in infos[id1]:
                    infos[id1][itemid] = {}
                if yid not in infos[id1][itemid]:
                    infos[id1][itemid][yid] = []
                # 查询是否在infos有过交互
                if len(infos[id1][itemid][yid])>0:
                    previousSharingTime = infos[id1][itemid][yid]
                    if testdate in previousSharingTime:
                        continue
                    else:
                        minLastShareTimePass = find_minimum_time_difference(testdate, previousSharingTime)
                        if minLastShareTimePass <= MaxLastShareTime:
                            print("测试id："+testdata['triple_id']+
                                "，对于邀请用户" + id1 + "和目标用户" + yid + "所分享的商品" + itemid + "本次分享时间：" + testdate.strftime(
                                    '%Y-%m-%d %H:%M:%S') + "，历史分享时间：" + ", ".join(
                                    [time.strftime('%Y-%m-%d %H:%M:%S') for time in previousSharingTime]) + "，时间差：" +
                                str(minLastShareTimePass) + "秒，跳过")
                            ShareTimePass.append(minLastShareTimePass)
                            continue
                ...
                if len(res) < 5:
                    # if yid in res:
                    #     lastShareTime = lastShareItemsTimes[id1][itemid][yid]
                    #     thisShareTime = testdate
                    #     # 计算最近一次分享的时间差
                    #     minLastShareTimePass = find_minimum_time_difference(thisShareTime, lastShareTime)
                    #     if minLastShareTimePass <= MaxLastShareTime:
                    #         ShareTimePass.append(minLastShareTimePass)
                    #         print(
                    #             "对于用户" + id1 + "和用户" + yid + "所分享的商品" + itemid + "本次分享时间：" + thisShareTime.strftime(
                    #                 '%Y-%m-%d %H:%M:%S') + "，历史分享时间：" + ", ".join(
                    #                 [time.strftime('%Y-%m-%d %H:%M:%S') for time in lastShareTime]) + "，时间差：" +
                    #             str(minLastShareTimePass) + "秒，跳过")
                    #         continue
                    wrt = f"{wrt}\t{xx}"
                    res.append(yid)
                    infos[id1][itemid][yid].append(testdate)
                    # if yid not in lastShareItemsTimes[id1][itemid]:
                    #     lastShareItemsTimes[id1][itemid][yid] = []
                    # if testdate not in lastShareItemsTimes[id1][itemid][yid]:
                    #     lastShareItemsTimes[id1][itemid][yid].append(testdate)
        ...
        with open('testres.txt', 'a') as ppr:
            ppr.write(wrt + '\n')
        submitres.append(subtemp)
    # 绘制直方图
    plt.hist(ShareTimePass, bins=10)  # 指定直方图的数据和分箱数
    plt.xlabel('Value')  # x轴标签
    plt.ylabel('Frequency')  # y轴标签
    plt.title('Histogram of ShareTimePass')  # 图表标题
    ...
    with open("submit.json", "wb") as f:  # 注意这里是"wb"，因为orjson.dumps返回的是bytes而不是str
        f.write(text)
